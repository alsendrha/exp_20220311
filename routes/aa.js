...
=======================================================================================================
-- 회원가입
INSERT INTO MEMBER2(USERDATE, USERID, USERPW, USERNAME, USERAGE)VALUES(CURRENT_DATE, 'a4', 'd', '가', 12);
SAVEPOINT s1; -- 저장지점

INSERT INTO MEMBER2(USERDATE, USERID, USERPW, USERNAME, USERAGE)VALUES(CURRENT_DATE, 'c', 'd', '가', 12);
SAVEPOINT s2; -- 저장지점
...
-- 회원정보수정 d라는 회원정보를 이름, 나이를 변경
UPDATE MEMBER2 SET USERNAME='aaa', USERAGE=111 WHERE USERID='d';
COMMIT;
SELECT * FROM MEMBER2;

-- 회원정보 일괄수정
UPDATE MEMBER2 SET USERNAME='aaa', USERAGE=111 WHERE USERID='a';
UPDATE MEMBER2 SET USERNAME='aaa', USERAGE=111 WHERE USERID='b';
UPDATE MEMBER2 SET USERNAME='aaa', USERAGE=111 WHERE USERID='c';
COMMIT;
...
-- 일괄삭제(조건이 2개, 아이디와 나이가 일치)
DELETE FROM MEMBER2 WHERE(USERID, USERAGE) IN (('a2',100),('a3',100),('a4',100));
COMMIT;
SELECT * FROM MEMBER2;

-- 조회하기(나이는 콤마, 날짜는 포멧을 "년월일 시")
SELECT M.USERID 아이디, M.USERNAME 이름,M.USERAGE *2 나이2배, TO_CHAR(M.USERAGE,'9,9,9')나이, TO_CHAR(M.USERDATE, 'YYYY-MM-DD HH24')날짜 FROM MEMBER2 M;

-- 페이지네이션
SELECT * FROM(
SELECT M.*, ROW_NUMBER() OVER(ORDER BY USERID ASC) 바보 FROM MEMBER2 M)M1
WHERE M1.바보 BETWEEN 1 AND 4;
-- BETWEEN ~부터 ~까지(페이지네이션에 이용)

-- 새로 만든 단축어는 다시 감싸야 이용가능

-- 페이지네이션 + 검색
SELECT M1.* FROM(
SELECT M.*, ROW_NUMBER() OVER(ORDER BY USERID ASC) ROWN FROM MEMBER2 M WHERE M.USERNAME LIKE '%'|| '가' || '%') M1
...
SELECT * FROM MEMBER2 M WHERE M.USERID LIKE '%'||'a'||'%';
SELECT M.*, ROW_NUMBER() OVER ( ORDER BY USERID DESC) ROWN FROM MEMBER2 M WHERE M.USERID LIKE  'a' || '%';
-- 나이가 10~40의 회원 수 조회
SELECT COUNT(*) FROM MEMBER2 WHERE USERAGE BETWEEN 10 AND 40;

-- 나이의 표준편차, 최대값 ...
SELECT STDDEV(USERAGE) 표준편차, MAX(USERAGE) 최대값, MIN(USERAGE) 최소값, SUM(USERAGE) 합, AVG(USERAGE) 평균 FROM MEMBER2 M;
-- MEMBER2의 USERID를 ITEM2테이블에 ITEMSELLER외래키 설정.
-- ITEM2 ITEMSELLER VARCHAR2() 컬럼추가
ALTER TABLE ITEM2 ADD ITEMSELLER VARCHAR2(30);
-- 외래키 연결
ALTER TABLE ITEM2 ADD CONSTRAINT FK_ITEM2_SELLER FOREIGN KEY(ITEMSELLER) REFERENCES MEMBER2(USERID);
-- 시퀀스 SEQ_ITEM2_ITEMNO 시작값 1001
CREATE SEQUENCE SEQ_ITEM2_ITEMNO START WITH 1001 INCREMENT BY 1 NOMAXVALUE;

-- ITEM2에 물품 3개 추가
INSERT INTO ITEM2(ITEMNO, ITEMNAME, ITEMPRICE, ITEMQTY, ITEMDATE, ITEMSELLER)
        VALUES(SEQ_ITEM2_ITEMNO.NEXTVAL,'핸드폰3', 2000, 30, CURRENT_DATE, 'a2');
COMMIT;
SELECT * FROM ITEM2;
SELECT * FROM MEMBER2;
-- MEMBER2+ITEM2를 조인하여 물품번호, 물품명, 가격, 수량, 판매자 아이디, 판매자 이름 조회
SELECT I.ITEMNO, I.ITEMNAME, I.ITEMPRICE, I.ITEMQTY, M.USERID, M.USERNAME  FROM MEMBER2 M, ITEM2 I WHERE I.ITEMSELLER=M.USERID;

-- 판매자가 a인 물품의 물품번호, 가격, 판매자이름, 나이, 조회
SELECT I.ITEMNO,I.ITEMPRICE, M.USERNAME, M.USERAGE FROM ITEM2 I, MEMBER2 M WHERE M.USERID=I.ITEMSELLER AND USERID='a';

-- 판매자별 물품수량, 재고수량합 조회
SELECT I.ITEMSELLER, COUNT(*), SUM(ITEMQTY) FROM ITEM2 I GROUP BY(ITEMSELLER);

SELECT ITEMSELLER, SUM(ITEMQTY) FROM ITEM2 I GROUP BY(ITEMSELLER);

-- 판매자별 물품수량, 재고수량 판매자 이름, 판매자 나이 조회
SELECT T1.*, M2.USERNAME, M2.USERAGE FROM MEMBER2 M2,
(SELECT I.ITEMSELLER, COUNT(*), SUM(ITEMQTY) FROM ITEM2 I GROUP BY(ITEMSELLER))T1
WHERE M2.USERID=T1.ITEMSELLER;
--------------------------------------------------------------------------------
SELECT M.*, I1.* FROM MEMBER2 M,(  
    SELECT ITEMSELLER, COUNT(*), SUM(ITEMQTY) FROM ITEM2 I GROUP BY(ITEMSELLER)
) I1 WHERE I1.ITEMSELLER = M.USERID;
--------------------------------------------------------------------------------
-- 재고수량합계가 100이상인 판매자별 물품수량 조회 
SELECT ITEMSELLER, SUM(ITEMQTY) FROM ITEM2 GROUP BY(ITEMSELLER) HAVING SUM(ITEMQTY) >= 10;

-- 판매자가 a인 물품의 물품수량, 물품가격평균 조회
SELECT T1.* FROM(
SELECT I.ITEMSELLER, COUNT(*),AVG(ITEMPRICE) FROM ITEM2 I GROUP BY(I.ITEMSELLER))T1
WHERE T1.ITEMSELLER='a';

SELECT ITEMSELLER, SUM(ITEMQTY), COUNT(*) FROM ITEM2 I WHERE ITEMSELLER ='a' GROUP BY(ITEMSELLER);

-- 판매자별 재고수량이 가장 높은 것 1개씩 조회
CREATE OR REPLACE VIEW ITEM2_VIEW AS
SELECT I.*, ROW_NUMBER() OVER(PARTITION BY ITEMSELLER ORDER BY ITEMQTY DESC) QTY FROM ITEM2 I;
SELECT * FROM ITEM2_VIEW WHERE QTY=1;

--ITEM2 + MEMBER2 + ORDER2 를 이용하여 주문내역 조회
-- (주문번호, 수량, 주문일자, 주문자 이름,주문자 나이)


-- VIEW 만들때 같은 명칭있으면 안됨 컬럼명
SELECT * FROM ORDER2 O, ITEM2 I, MEMBER2 M WHERE I.ITEMNO=O.ITEMNO AND M.USERID=O.USERID; 

SELECT M.*, I.*, O.* FROM MEMBER2 M, ITEM2 I, ORDER2 O WHERE M.USERID=O.USERID AND I.ITEMNO=O.ITEMNO;

SELECT * FROM ORDER2;
INSERT INTO ORDER2(ORDNO, ORDCNT, ORDDATE, ITEMNO, USERID)VALUES(SEQ_ORDER2_NO.NEXTVAL, 4, CURRENT_DATE, 1004, 'C');

CREATE SEQUENCE SEQ_ORDER2_NO START WITH 1 INCREMENT BY 1 NOMAXVALUE;

-- 일자별 주문수량, 주문총합 물품명, 가격, 재고수령
